...
> | `ЗагрузитьРасширениеВРежимеПредприятия` (проверка) | `vrunner cfe check-applicability` |
>
...
+ **ЗагрузитьРасширениеВРежимеПредприятия** позволяет подключать расширение в режиме предприятия и получать результат ошибки, а также выполнять только проверку применимости уже установленных расширений (параметр `ЗапуститьТолькоПроверкуПрименимостиРасширений`). Предназначено для конфигураций, основанных на БСП. Проверка применимости доступна командой `vrunner cfe check-applicability`.
