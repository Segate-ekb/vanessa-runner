...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды, "--versions", 
		...
		|		Если каталог --out пуст, в него выгружаются только объекты, измененные относительно файла версии.
		|		Если каталог --out уже содержит выгрузку (ConfigDumpInfo.xml) или не пуст, выгрузка в нем обновляется:
		|		перезаписываются только измененные объекты, изменения вычисляются относительно файла версии.");
