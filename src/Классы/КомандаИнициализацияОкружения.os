...
Процедура ИнициализироватьБазуДанных(Знач ВерсияПлатформы,
	...
	Иначе
		// Тут мы будем только если база серверная,
		// т.к. БазуСоздавали устанавливается в Истину только в случае фаайловой базы
		МенеджерСборки.УстановитьКонтекст(ДанныеПодключения, Истина);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПутьКФайлуВыгрузки) Тогда
		...
		МенеджерСборки.УстановитьКонтекст(ДанныеПодключения, Не ОбщиеМетоды.ЭтоФайловаяИБ(СтрокаПодключения));
	КонецЕсли;	
