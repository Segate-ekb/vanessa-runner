...
Процедура УстановитьКонтекст(ДанныеПодключения, Серверная = Ложь) Экспорт

	Если Серверная Тогда
		Лог.Отладка("Установим контекст серверной ИБ");
		УправлениеИБ.УстановитьПараметрыСервернойИБ(ДанныеПодключения.СУБД_Тип,
...
Процедура Конструктор(Знач ДанныеПодключения, Знач ПараметрыКоманды) Экспорт
	...
	Иначе
		...
		УстановитьКонтекст(ДанныеПодключения, Не ОбщиеМетоды.ЭтоФайловаяИБ(СтрокаПодключенияИБ));	
