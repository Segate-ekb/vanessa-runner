...
Процедура РазобратьНаИсходникиТекущуюКонфигурацию(КаталогВыгрузки, Знач ФайлВерсии = "",
	...
	ФайлВерсии = ?(ТолькоИзмененные И ФС.ФайлСуществует(ФайлВерсии), ФайлВерсии, "");
	Синхронизировать = ПодготовитьКаталогВыгрузки(КаталогВыгрузки, ФайлВерсии);
...
// Определяет режим выгрузки в каталог: обновление существующей выгрузки (--sync) или выгрузка в пустой каталог
// (полная либо изменений относительно файла версий, --base). ibcmd не позволяет сочетать --base и --sync,
// поэтому при обновлении изменения вычисляются по ConfigDumpInfo.xml каталога (переданный файл версий копируется туда).
//
// Параметры:
//   КаталогВыгрузки - Строка - каталог выгрузки
//   ФайлВерсии - Строка - путь к файлу версий; очищается, если выгрузка будет обновляться
//
// Возвращаемое значение:
//   Булево - Истина, если нужно обновлять существующую выгрузку (--sync)
//
Функция ПодготовитьКаталогВыгрузки(Знач КаталогВыгрузки, ФайлВерсии)

	Если НЕ ОбщиеМетоды.ЭтоКаталогСуществующейВыгрузки(КаталогВыгрузки, ФайлВерсии) Тогда
		Возврат Ложь;
	КонецЕсли;
