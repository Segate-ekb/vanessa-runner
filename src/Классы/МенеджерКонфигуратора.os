...
Процедура РазобратьНаИсходникиТекущуюКонфигурацию(КаталогВыгрузки, Знач ФайлВерсии = "",
	...
	ФайлВерсии = ?(ТолькоИзмененные, ФайлВерсии, "");
	Если ОбновлятьСуществующуюВыгрузку(КаталогРаспаковки, ФайлВерсии) Тогда
		ОбновитьСуществующуюВыгрузку(КаталогРаспаковки, ФайлВерсии);
	Иначе
...
Процедура РазобратьРасширениеНаИсходники(КаталогВыгрузки, ИмяРасширения,
	...
	ФайлВерсии = ?(Синхронизировать, ФайлВерсии, "");

	Если ОбновлятьСуществующуюВыгрузку(КаталогВыгрузки, ФайлВерсии) Тогда
		ОбновитьСуществующуюВыгрузку(КаталогВыгрузки, ФайлВерсии, ИмяРасширения);
		Возврат;
	КонецЕсли;
...
// Выгрузка изменений относительно файла версий (-configDumpInfoForChanges) штатно работает только в пустой каталог.
// Если каталог уже содержит выгрузку, ее нужно обновлять (-update -force), иначе платформа сообщает "Каталог не пуст".
//
// Параметры:
//   КаталогВыгрузки - Строка - каталог выгрузки
//   ФайлВерсии - Строка - путь к файлу версий, может быть пустым
//
// Возвращаемое значение:
//   Булево - Истина, если нужно обновлять существующую выгрузку
//
Функция ОбновлятьСуществующуюВыгрузку(Знач КаталогВыгрузки, Знач ФайлВерсии)

	Возврат УправлениеКонфигуратором.ИспользуемаяВерсияПлатформыСтаршеИлиРавна("8.3.10")
		И ОбщиеМетоды.ЭтоКаталогСуществующейВыгрузки(КаталогВыгрузки, ФайлВерсии);

КонецФункции

// Обновляет существующую выгрузку конфигурации или расширения (/DumpConfigToFiles -update -force).
// Изменения вычисляются относительно ConfigDumpInfo.xml в каталоге выгрузки (переданный файл версий копируется туда).
// Если инкрементальную выгрузку выполнить невозможно (файл версий не соответствует ИБ),
// платформа за счет -force выполняет полную выгрузку в существующий каталог.
//
// Параметры:
//   КаталогВыгрузки - Строка - каталог существующей выгрузки
//   ФайлВерсии - Строка - путь к ConfigDumpInfo.xml (или его копии) для вычисления изменений, может быть пустым
//   ИмяРасширения - Строка - имя расширения; пустая строка для основной конфигурации
//
Процедура ОбновитьСуществующуюВыгрузку(Знач КаталогВыгрузки, Знач ФайлВерсии, Знач ИмяРасширения = "")

	Лог.Информация("Обновляем существующую выгрузку в каталоге %1", КаталогВыгрузки);
	ОбщиеМетоды.ПодготовитьФайлВерсийВКаталогеВыгрузки(КаталогВыгрузки, ФайлВерсии);

	Конфигуратор = УправлениеКонфигуратором();
	Параметры = Конфигуратор.ПолучитьПараметрыЗапуска();
	Параметры.Добавить(СтрШаблон("/DumpConfigToFiles %1", ОбщиеМетоды.ОбернутьПутьВКавычки(КаталогВыгрузки)));
	Если ЗначениеЗаполнено(ИмяРасширения) Тогда
		Параметры.Добавить(СтрШаблон("-Extension ""%1""", ИмяРасширения));
	КонецЕсли;
	Параметры.Добавить(СтрШаблон("-format %1", РежимВыгрузкиКонфигурации.Иерархический));
