...
Функция ПолучитьФайлыИзGitDiff(Знач ПутьИсходников)
	...
	Для каждого ИзмененныйФайл Из МассивФайлов Цикл		
		Лог.Информация("  Изменился файл: %1", ИзмененныйФайл);
		...
		Если СтрНачинаетсяС(ИзмененныйФайл, ОтносительныйПутьИсходников)
			...
			Лог.Информация("  	После корректировки формы: %1", ИзмененныйФайл);
			ИзмененныйФайл = СтрЗаменить(ИзмененныйФайл, "/", ПолучитьРазделительПути());
			Лог.Информация("  	После замены символов разделителя пути: %1", ИзмененныйФайл);
			ТекущаяСтрока = ОбъединитьПути(ТекущийКаталог, ИзмененныйФайл);
			Лог.Информация("  	Текущая строка: %1", ТекущаяСтрока);
	...
	КонецЦикла;
	Лог.Информация("  	СтрокаИзмененныхФайлов из git: %1", СтрокаИзмененныхФайлов);
	Возврат СтрокаИзмененныхФайлов;
КонецФункции
...
Функция ПолучитьИзмененныеВКонфигурацииФайлыСМоментаПоследнейЗагрузки(Знач ПутьИсходников, МенеджерСборки)
	...
	СтрокаИзмененныхФайлов = "";
	ПутьКФайлуЭталона = ОбъединитьПути(КорневойПутьПроекта, ПутьИсходников, ИмяФайлаЭталонаConfigdumpInfo());
	...
	КонецЕсли;
	Сообщить("  	СтрокаИзмененныхФайлов из Конфигуратора: "+ СтрокаИзмененныхФайлов);
	Возврат СтрокаИзмененныхФайлов;
КонецФункции
...
Процедура УстановитьQuotepath(Репозиторий)
	...
	Если Не Нрег(СокрЛП(Репозиторий.ПолучитьНастройку("core.quotePath"))) = "false" Тогда
		Лог.Информация("Для корректной работы <--git-increment> необходима настройка git config core.quotePath false");
...
Процедура ЗаписатьlastUploadedConfigDumpInfo(Знач ПутьИсходников) // BSLLS:LatinAndCyrillicSymbolInWord-off
	...
	ФайлПреемник = ОбъединитьПути(ПутьИсходников, ИмяФайлаЭталонаConfigdumpInfo());
...
Функция ИмяФайлаЭталонаConfigdumpInfo()
