...
Процедура УстановитьQuotepath(Репозиторий)
	...
	Если СокрЛП(Репозиторий.ПолучитьНастройку("core.quotePath")) = "true" Тогда
		Лог.Информация("Для корректной работы <--git-increment> необходима настройка git config core.quotePath false");
		Репозиторий.УстановитьНастройку("core.quotePath", "true", РежимУстановкиНастроекGit.Локально);
	КонецЕсли;
