...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	Парсер.ДобавитьПараметрФлагКоманды(ОписаниеКоманды, "--git-increment",
		СтрШаблон("Инкрементальная загрузка по git diff
		|	Схема работы
		| 		При загрузке в каталоге исходников (--src) ищется файл %1.
		| 		В нем хранится хэш коммита, который УЖЕ загружен в ИБ (а не коммит, который нужно загрузить).
		| 		Если файл найден, загружаются файлы, измененные от этого коммита до текущего состояния
		| 		рабочей копии (git diff <хэш>), плюс объекты, измененные в ИБ с момента последней загрузки
		| 		(по файлу %2).
		| 		Если файл не найден, происходит полная загрузка.
		| 		После успешной загрузки файл %1 обновляется хэшем HEAD, а %2 - копией ConfigDumpInfo.xml.
		| 		Файлы %1 и %2 описывают состояние конкретной локальной ИБ - добавьте их в .gitignore.
		|", ИмяФайлаПредыдущегоГитКоммита(), ИмяФайлаLastUploadedConfigDumpInfo()));
...
Функция ПолучитьСтрокуИзмененныхФайлов(Знач ПутьИсходников, МенеджерСборки)
	...
	Если ПустаяСтрока(Хэш) Тогда
		Лог.Информация("Файл %1 не найден или пуст, будет выполнена полная загрузка.",
			ФайлПредыдущегоГитКоммита(ПутьИсходников).ПолноеИмя);
		Возврат "";
	КонецЕсли;

	ИзмененныеФайлыGitDiff = ПолучитьФайлыИзGitDiff(ПутьИсходников, Хэш);
	...
	Если ИзмененныеФайлыИзКонфигурации = "FullDump" Тогда
		Лог.Информация("Невозможно вычислить изменения конфигурации с момента последней загрузки,
			|будет выполнена полная загрузка.");
		Возврат "";
	КонецЕсли;

	МассивФайлов = НормализоватьСписокФайловДляЗагрузки(ИзмененныеФайлыGitDiff + ИзмененныеФайлыИзКонфигурации);

	Если МассивФайлов.Количество() = 0 Тогда
		Лог.Информация("Не найдено измененных файлов для инкрементальной загрузки, будет выполнена полная загрузка.");
		Возврат "";
	КонецЕсли;

	КоличествоФайлов = МассивФайлов.Количество();
	ИзмененныеФайлыЛог = СтрСоединить(МассивФайлов, Символы.ПС);
	...
	Возврат СтрСоединить(МассивФайлов, ";");

КонецФункции

// Приводит список файлов для частичной загрузки к единому виду:
// абсолютные пути, без служебных файлов и без повторов.
//
// Параметры:
//   СтрокаИзмененныхФайлов - Строка - пути к файлам, разделенные ";"
//
// Возвращаемое значение:
//   Массив из Строка - полные пути к файлам
//
Функция НормализоватьСписокФайловДляЗагрузки(Знач СтрокаИзмененныхФайлов)

	Результат = Новый Массив;
	УжеДобавленные = Новый Соответствие;

	Для Каждого ПутьКФайлу Из СтрРазделить(СтрокаИзмененныхФайлов, ";", Ложь) Цикл

		ПутьКФайлу = СокрЛП(ПутьКФайлу);
		Если ПустаяСтрока(ПутьКФайлу) Или ФайлВСпискеИсключений(ПутьКФайлу) Тогда
			Продолжить;
		КонецЕсли;

		ПолныйПуть = ОбщиеМетоды.ПолныйПуть(ПутьКФайлу);
		Ключ = ?(ПараметрыСистемы.ЭтоWindows, НРег(ПолныйПуть), ПолныйПуть);
		Если УжеДобавленные[Ключ] = Неопределено Тогда
			УжеДобавленные.Вставить(Ключ, Истина);
			Результат.Добавить(ПолныйПуть);
		КонецЕсли;
...
Функция ПолучитьФайлыИзGitDiff(Знач ПутьИсходников, Знач Хэш)
	...
	Лог.Информация("Инкрементальная загрузка по git: последний загруженный коммит <%1>, HEAD <%2>",
		Хэш, ХэшТекущегоКоммита(Репозиторий));
	...
	Для каждого ИзмененныйФайл Из МассивФайлов Цикл
		...
		Если СтрНачинаетсяС(ИзмененныйФайл, ОтносительныйПутьИсходников)
			...
			Если Новый Файл(ТекущаяСтрока).Существует() Тогда
...
Функция ХэшТекущегоКоммита(Репозиторий) // BSLLS:Typo-off
	...
	Возврат СокрЛП(Репозиторий.ПолучитьВыводКоманды());
