...
Функция ПолучитьФайлыИзGitDiff(Знач ПутьИсходников)
	...
	КоманднаяСтрока = СтрШаблон("git diff --name-only %1", Хэш);
	...
	Пока Процесс.ПотокВывода.ЕстьДанные Цикл
		...
		Если СтрНачинаетсяС(СтрокаВывода, ОтносительныйПутьИсходников)
			...
			СтрокаВывода = СтрЗаменить(СтрокаВывода, ОтносительныйПутьИсходников, "");
...
Процедура ЗаписатьlastUploadedConfigDumpInfo(Знач ПутьИсходников)
