...
Процедура УстановитьQuotepath(Репозиторий)
	...
	ПараметрыКоманды = Новый Массив;
	ПараметрыКоманды.Добавить("config --get core.quotepath");
	Репозиторий.ВыполнитьКоманду(ПараметрыКоманды);


	Если СокрЛП(Репозиторий.ПолучитьВыводКоманды()) = "true" Тогда
		...
		ПараметрыКоманды.Добавить("core.quotepath false");
		Репозиторий.ВыполнитьКоманду(ПараметрыКоманды);
	КонецЕсли;
