...
Процедура УстановитьQuotepath(Репозиторий)
	...
	ПараметрыКоманды.Добавить("--get core.quotepath");
	Репозиторий.ВыполнитьКоманду(ПараметрыКоманды);

	Если Репозиторий.ПолучитьВыводКоманды() = "True" Тогда
		Лог.Информация("Для корректной работы <--git-increment> необходимо значение git config core.quotepath false");
		ПараметрыКоманды = Новый Массив;
		ПараметрыКоманды.Добавить("config");
