...
Процедура ДополнитьАргументыИзФайлаНастроек(Знач Команда, ЗначенияПараметров, Знач НастройкиИзФайла) Экспорт
	...
	ДополнитьСоответствиеСУчетомПриоритета(ЗначенияПараметров, НастройкиИзФайла.Получить(Команда), Истина);
...
Процедура ДополнитьСоответствиеСУчетомПриоритета(КоллекцияОсновная, Знач КоллекцияДоп, ПерезаписыватьИстину = Ложь) Экспорт
	...
	Для Каждого Элемент Из КоллекцияДоп Цикл
		...
		ИначеЕсли ТипЗнч(Значение) = Тип("Булево") И (НЕ Значение ИЛИ ПерезаписыватьИстину) Тогда
			Если ТипЗнч(ЗначениеПодстановки) = Тип("Строка") Тогда
				...
				КонецЕсли;
			ИначеЕсли ТипЗнч(ЗначениеПодстановки) = Тип("Булево") Тогда
				КоллекцияОсновная.Вставить(Элемент.Ключ, ЗначениеПодстановки);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьБлокIbcmd(Знач Парсер, ОписаниеКоманды) Экспорт
	...
	Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды, "--dbms-type", "Тип сервера СУБД");
