...
// Каталог уже содержит выгрузку конфигурации (в нем есть ConfigDumpInfo.xml) либо он не пуст и для него
// передан файл версий. В этом случае выгрузку нужно обновлять, а не выгружать в каталог заново.
//
// Параметры:
//   КаталогВыгрузки - Строка - каталог выгрузки
//   ФайлВерсии - Строка - путь к файлу версий (ConfigDumpInfo.xml или его копия), может быть пустым
//
// Возвращаемое значение:
//   Булево
//
Функция ЭтоКаталогСуществующейВыгрузки(Знач КаталогВыгрузки, Знач ФайлВерсии = "") Экспорт

	Если ФС.ФайлСуществует(ОбъединитьПути(КаталогВыгрузки, "ConfigDumpInfo.xml")) Тогда
		Возврат Истина;
	КонецЕсли;

	Возврат ЗначениеЗаполнено(ФайлВерсии) И ФС.ФайлСуществует(ФайлВерсии) И НЕ КаталогПуст(КаталогВыгрузки);

КонецФункции

// Копирует переданный файл версий в каталог выгрузки как ConfigDumpInfo.xml (если это другой файл):
// относительно него платформа вычисляет изменения при обновлении существующей выгрузки.
//
// Параметры:
//   КаталогВыгрузки - Строка - каталог существующей выгрузки
//   ФайлВерсии - Строка - путь к файлу версий, может быть пустым
//
Процедура ПодготовитьФайлВерсийВКаталогеВыгрузки(Знач КаталогВыгрузки, Знач ФайлВерсии) Экспорт

	Если ПустаяСтрока(ФайлВерсии) Или НЕ ФС.ФайлСуществует(ФайлВерсии) Тогда
		Возврат;
	КонецЕсли;

	ФайлВерсииВКаталоге = ОбъединитьПути(КаталогВыгрузки, "ConfigDumpInfo.xml");
	Если ЭтоОдинИТотЖеФайл(ФайлВерсии, ФайлВерсииВКаталоге) Тогда
		Возврат;
	КонецЕсли;

	ПолучитьЛог().Отладка("Изменения вычисляются относительно файла версий %1", ФайлВерсии);
	КопироватьФайл(ФайлВерсии, ФайлВерсииВКаталоге);

КонецПроцедуры

Функция КаталогПуст(Знач Каталог)

	Если НЕ ФС.КаталогСуществует(Каталог) Тогда
		Возврат Истина;
	КонецЕсли;

	Возврат НайтиФайлы(Каталог, ПолучитьМаскуВсеФайлы(), Ложь).Количество() = 0;

КонецФункции

Функция ЭтоОдинИТотЖеФайл(Знач Путь1, Знач Путь2)

	ПолноеИмя1 = Новый Файл(Путь1).ПолноеИмя;
	ПолноеИмя2 = Новый Файл(Путь2).ПолноеИмя;

	Возврат ?(ПараметрыСистемы.ЭтоWindows, НРег(ПолноеИмя1) = НРег(ПолноеИмя2), ПолноеИмя1 = ПолноеИмя2);
