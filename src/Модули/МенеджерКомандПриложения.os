...
// Выполняет обогащение параметров командной строки параметрами из альтернативных источников.
...
КонецПроцедуры // ДополнитьКоллекциюИзФайлаНастроек

Процедура ОбогатитьКоллекцию(КоллекцияОсновная,
												Знач КоллекцияДоп,
												НеявнаяЛожь = Ложь)
	...
	Для Каждого Элемент Из КоллекцияДоп Цикл
		...
		Если ТипЗнч(Значение) = Тип("Булево") Тогда
			Если Не ЗначениеПодстановки И НеявнаяЛожь Тогда
				...
				Лог.Отладка("Параметр <%1> не был явно указан в коллекции. Переопределение выполнено не будет", Элемент.Ключ);
				Продолжить;
			КонецЕсли;
		...
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры // ОбогатитьКоллекцию
