...
- [Описание](#описание-1)
  ...
  - [Использование](#использование)
    ...
    - [Дополнительная настройка различных команд](#дополнительная-настройка-различных-команд)
      ...
      - [Инкрементальная загрузка `update-dev --git-increment`](#инкрементальная-загрузка-update-dev---git-increment)
...
<a id="markdown-инкрементальная-загрузка-update-dev---git-increment" name="инкрементальная-загрузка-update-dev---git-increment"></a>
#### Инкрементальная загрузка `update-dev --git-increment`

Ключ `--git-increment` команды `update-dev` загружает в ИБ не все исходники, а только изменившиеся файлы.
Что именно изменилось, определяется по двум служебным файлам в каталоге исходников (`--src`):

- `lastUploadedCommit.txt` - хэш коммита, который **уже загружен** в ИБ (а не коммит, который нужно загрузить).
  Загружаются файлы из `git diff <lastUploadedCommit>` - то есть все изменения от этого коммита до текущего состояния
  рабочей копии, включая незакоммиченные.
- `lastUploadedConfigDumpInfo.xml` - копия `ConfigDumpInfo.xml` на момент последней загрузки. По ней дополнительно
  определяются объекты, изменившиеся в самой ИБ (например, через конфигуратор), чтобы их перезагрузить из исходников.

Правила работы:

1. Если `lastUploadedCommit.txt` отсутствует или пуст, выполняется полная загрузка.
2. Если изменений не найдено или изменения вычислить невозможно, выполняется полная загрузка.
3. После успешной загрузки `lastUploadedCommit.txt` обновляется хэшем текущего `HEAD`,
   а `lastUploadedConfigDumpInfo.xml` - актуальной копией `ConfigDumpInfo.xml`.
4. Оба файла описывают состояние конкретной локальной ИБ, поэтому их **не нужно версионировать** - добавьте в `.gitignore`.
   `ConfigDumpInfo.xml` тоже стоит исключить из репозитория, если он отражает состояние локальной ИБ.
5. Если в `lastUploadedCommit.txt` указать текущий `HEAD`, изменений для загрузки не будет и пройдет полная загрузка.
   Если указан слишком старый коммит, загрузка будет большой.
...
ИБ находится в состоянии `B`, нужно догрузить `C` и `D`: в `lastUploadedCommit.txt` должен быть `B`.
После загрузки runner сам запишет туда `D`.

Типовой локальный workflow:

```bash
# 1. Один раз после полной загрузки (или синхронизации ИБ с репозиторием)
git rev-parse --short HEAD > src/cf/lastUploadedCommit.txt

# 2. Служебные файлы - в .gitignore
cat >> .gitignore <<'EOF'
src/cf/lastUploadedCommit.txt
src/cf/lastUploadedConfigDumpInfo.xml
EOF

# 3. После новых коммитов (или правок в рабочей копии) догружаем изменения
vrunner update-dev --src src/cf --git-increment --ibconnection /Fbuild/ib
```

В логе команды выводятся хэши последнего загруженного коммита и `HEAD`, а также список файлов,
которые будут загружены. Служебные файлы и повторы из списка исключаются автоматически.
